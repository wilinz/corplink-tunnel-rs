...
impl Client {
    ...
    pub async fn connect_vpn(&mut self) -> Result<WgConf> {
        ...
        for v in &vpn_info {
            log::info!("[node] ip={} vpn_port={} api_port={} mode={} id={}", v.ip, v.vpn_port, v.api_port, v.protocol_mode, v.id);
        }
        let skip_ip = std::env::var("CORPLINK_SKIP_IP").ok();
        let filtered_vpn = vpn_info
            .into_iter()
            .filter(|vpn| {
                if let Some(ref sip) = skip_ip {
                    if &vpn.ip == sip { log::info!("skip node {} by CORPLINK_SKIP_IP", vpn.ip); return false; }
                }
        ...
        // [POC] dump the full wg conf so a pure-Rust datapath prototype can reuse it
        if let Ok(path) = std::env::var("WG_CONF_DUMP") {
            let j = serde_json::json!({
                "address": wg_conf.address, "peer_address": wg_conf.peer_address,
                "mtu": wg_conf.mtu, "private_key": wg_conf.private_key,
                "peer_key": wg_conf.peer_key, "dns": wg_conf.dns,
                "protocol": wg_conf.protocol, "allowed_ips": wg_conf.allowed_ips,
            });
            let _ = std::fs::write(&path, serde_json::to_string_pretty(&j).unwrap());
            log::info!("[POC] wg conf dumped to {path}");
            if std::env::var("WG_CONF_DUMP_EXIT").is_ok() {
                log::info!("[POC] exit after dump (key registered, no WG session started)");
                std::process::exit(0);
            }
        }
        Ok(wg_conf)
    ...
    // quota (servers cap the number of concurrent terminals).
